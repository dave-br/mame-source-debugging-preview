-- This is an example script which displays an overlay showing
-- the NitrOS-9 modules that have been loaded and their individual
-- memory maps. This also automates the process of determining where
-- a module that you wish to debug has been loaded, and then executing
-- the sdoffset command for you with the appropriate argument.

-- To load this script add the following to the end of your MAME command line:
-- 			-script path/to/OS9.lua

-- The script will initially load disabled, and you may toggle its enabled state
-- by pressing the backslash key (\) while focus is on the emulated machine's window.
-- You should not toggle the script to the enabled state until after you have begun
-- loading NitrOS-9.

-- In order to have the script properly execute the sdoffset command, change the
-- value of the debugged_module_name variable below to the name of the module
-- you wish to debug.

-- Just a little something to let us know you loaded
print("lua says... " .. emu.app_name() .. " " .. emu.app_version())

-- Set to module name you want to debug
debugged_module_name = "hw"
...
function draw_overlay()
    ...
    for pid = 1,255,1 do
        ...
        -- Information is finally gathered; time to process
        if procName ~= '' then
            draw_map(
                mapNum, 
            ...
            mapNum = mapNum + 1
			if (not offsetApplied) and debugged_module_name == procName then
				adjust_offset_if_necesary(procName, moduleAddr)
			end
        end
        ::continue::
    end
...
function adjust_offset_if_necesary(procName, offset)
	if offsetApplied or debugged_module_name ~= procName then
		return
	end

	-- Very bare-bones breakpoint adjustment.  All attributes
	-- but the address are lost

	-- To hold original set of breakpoints
	bps = {}
	
	-- Grab existing addresses
	for idx,bp in pairs(device_debug:bplist()) do
		bps[idx] = tonumber(bp.address)
	end
	
	-- Clear all bps
	device_debug:bpclear()
	
	-- Add them back with the offset
	for idx,bp in pairs(bps) do
		print(string.format("calling bpset(%d)", bp + offset))
		device_debug:bpset(bp + offset, "", "")
	end
	
	-- Inform rest of debugger about the offset
	debugger:command(string.format("sdoffset #%d", offset))
	offsetApplied = true
end
